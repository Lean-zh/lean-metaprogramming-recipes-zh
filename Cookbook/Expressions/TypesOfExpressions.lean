...
这些由 `proj` 构造子给出，表示结构体投影。它们由一个结构体名、一个字段索引和一个结构体表达式组成。例如，如果我们有一个带字段 `x` 和 `y` 的结构体 `Point`，那么表达式 `p.x` 会被表示为 `Lean.Expr.proj Point 0 (Lean.Expr.const p [])`，而表达式 `p.y` 会被表示为 `Lean.Expr.proj Point 1 (Lean.Expr.const p [])`。
